...
namespace AdminLayer.Controllers.Staff
{
    ...
    public class QuestsController : ControllerBase
    {
        ...
        {
            var dto = await _questAdminService.GetAsync(id, ct);
            if (dto == null)
        ...
        public async Task<IActionResult> Toggle(long id, CancellationToken ct)
        {
            try
            {
                await _questAdminService.ToggleStatusAsync(id, ct);
                return Ok(ApiResponse<object>.SuccessResponse(null, "Thay đổi trạng thái Quest thành công."));
            }
